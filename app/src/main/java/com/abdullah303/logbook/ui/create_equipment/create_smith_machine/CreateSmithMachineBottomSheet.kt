...
@Composable
fun CreateSmithMachineBottomSheet(
    modifier: Modifier = Modifier,
    onDismiss: () -> Unit = {},
    onSmithMachineCreated: (SmithMachineConfiguration) -> Unit = { _ -> },
    sheetState: SheetState,
    maxHeightFraction: Float = 0.75f,
    viewModel: CreateSmithMachineViewModel = hiltViewModel()
) {
    ...
    val weightValues = remember(uiState.weightUnit) {
        when (uiState.weightUnit) {
            WeightUnit.KG -> (0..200).map { it * 0.5f } // 0kg to 100kg in 0.5kg increments
            WeightUnit.LB -> (0..440).map { it * 0.5f } // 0lb to 220lb in 0.5lb increments
        }
    }

    GenericBottomSheet(
        title = "create smith machine",
        modifier = modifier,
        onDismiss = {
        ...
        Column(
            ...
            Surface(
                ...
                Column(
                    ...
                    // smith machine name input card
                    FormInputCard(
                        title = "smith machine name",
                        value = uiState.name,
                        onValueChange = { name ->
                            errorMessage = null // clear any previous error when user types
                            viewModel.updateName(name)
                        },
                        placeholder = "enter smith machine name..."
                    )

                    // bar weight selection card with unit toggle
                    FormSelectionCard(
                        title = "bar weight",
